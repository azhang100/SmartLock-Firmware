...
void motorOut(int rotationalVelocity){
  ...
  {analogWrite(MOTOR_B_PWM, dutyCycle);analogWrite(MOTOR_B_DIR, 0);debug.println("forwards");}
  
  else if (dutyCycle < -MOTOR_DUTY_BUFFER)
  {analogWrite(MOTOR_B_DIR, -dutyCycle);analogWrite(MOTOR_B_PWM, 0);debug.println("backwards");}
  
  else
  {analogWrite(MOTOR_B_PWM, 0);analogWrite(MOTOR_B_DIR, 0);debug.println("stop");}
}
