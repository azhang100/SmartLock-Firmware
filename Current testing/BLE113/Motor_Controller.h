...
void controlMotor(char command)
{
  ...
  {  desiredAngle = theMotorConfig.unlockedAngle; desiredDirection = theMotorConfig.turnDirection; motorActive = true;
      debug.println("please unlock"); debug.println(theMotorConfig.unlockedAngle);
  }
  else if (command=='l' && isLocked==false)
  {
    desiredAngle = theMotorConfig.lockedAngle; desiredDirection = -theMotorConfig.turnDirection; motorActive = true;
     debug.println("please lock"); debug.println(theMotorConfig.lockedAngle);
  }  
  
  while(motorActive == true)
  {
    angleFromAccelerometer = getAngle();
      debug.println("current angle:");     debug.println(angleFromAccelerometer);
      debug.println("desired angle:");     debug.println(desiredAngle);
        
    //Moves slightly closer to desiredAngle in desiredDirection
    if (abs(angleFromAccelerometer - desiredAngle) < MOTOR_ANGLE_BUFFER )
    {
      // reached position
         debug.println("yay reached my destination");
      motorOut(0);
      motorActive = false;
       if (command=='u') 
       {  
...
void rotateMotor360()
{
  ...
  while(motorActive == true)
  {
    angleFromAccelerometer = getAngle();
      debug.println("current angle:");     debug.println(angleFromAccelerometer);
      debug.println("desired angle:");     debug.println(initialAngleFromAccelerometerMinusBuffer);
