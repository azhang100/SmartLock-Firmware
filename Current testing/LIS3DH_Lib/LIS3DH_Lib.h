...
#define        SLAVE_ADDR      0x19   //Slave address of Accelerometer//If SDO/SA0 is high '1' then the address changes to 0b0011001// If its low then the address is 0b0011000 
...
char getPosition(){
  char pos = 'j';
  if (getAngle() <= CLOSE_ANGLE) {pos = 'c';}
  else if (getAngle() >= OPEN_ANGLE) {pos = 'o';}
